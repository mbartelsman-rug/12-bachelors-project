...
module type UNSPEC = sig
  ...
  and env_t = (string_t * expr_t) queue_t * (value_t queue_t) dict_t * value_t dict_t
  and func_env_t = value_t dict_t
  and func_t = string_t * expr_t * ((string_t * expr_t) queue_t * (value_t queue_t) dict_t * value_t dict_t)
  and inbox_t = value_t queue_t
  and name_t = string_t
  and pair_t = value_t * value_t
  and pid_t = string_t
  and rec_func_t = string_t * string_t * expr_t * ((string_t * expr_t) queue_t * (value_t queue_t) dict_t * value_t dict_t)
...
module Unspec (M: MONAD) (T: TYPES) = struct
  ...
  let actor_self _ = raise (NotImplemented "actor_self")
