...
module type UNSPEC = sig
  ...
  and chan_env_t = (id_t, value_t queue_t) dict_t
  and chan_id_t = id_t
  and env_t = expr_t queue_t * (id_t, value_t queue_t) dict_t * (name_t, value_t) dict_t
  and func_env_t = (name_t, value_t) dict_t
  and func_t = name_t * expr_t * (expr_t queue_t * (id_t, value_t queue_t) dict_t * (name_t, value_t) dict_t)
  and pair_t = value_t * value_t
  and rec_func_t = name_t * name_t * expr_t * (expr_t queue_t * (id_t, value_t queue_t) dict_t * (name_t, value_t) dict_t)
  and thread_pool_t = expr_t queue_t

  val dict_drop: ('k, 'v) dict_t * 'k -> (('k, 'v) dict_t) M.t
  val dict_has_no: ('k, 'v) dict_t * 'k -> (('k, 'v) dict_t) M.t
  val dict_has_some: ('k, 'v) dict_t * 'k -> (('k, 'v) dict_t) M.t
  val dict_is_empty: ('k, 'v) dict_t -> (('k, 'v) dict_t) M.t
  val dict_is_empty_not: ('k, 'v) dict_t -> (('k, 'v) dict_t) M.t
  val dict_new: unit -> (('k, 'v) dict_t) M.t
  val dict_read: ('k, 'v) dict_t * 'k -> 'v M.t
  val dict_write: ('k, 'v) dict_t * 'k * 'v -> (('k, 'v) dict_t) M.t
  val id_new: unit -> id_t M.t
  ...
  val int_new: unit -> value_t M.t
  val int_sub: int_t * int_t -> value_t M.t
  val name_eq: name_t * name_t -> unit M.t
...
module Unspec (M: MONAD) (T: TYPES) = struct
  ...
  let dict_has_no _ = raise (NotImplemented "dict_has_no")
  let dict_has_some _ = raise (NotImplemented "dict_has_some")
  let dict_is_empty _ = raise (NotImplemented "dict_is_empty")
  let dict_is_empty_not _ = raise (NotImplemented "dict_is_empty_not")
  ...
  let id_new _ = raise (NotImplemented "id_new")
  ...
  let int_new _ = raise (NotImplemented "int_new")
  let int_sub _ = raise (NotImplemented "int_sub")
  let name_eq _ = raise (NotImplemented "name_eq")
  ...
  let queue_is_not_empty _ = raise (NotImplemented "queue_is_not_empty")
  ...
  and func_subst_in =
    ...
    M.branch [
      (function () ->
        begin match expr with
        | Var name ->
            let* () = apply1 name_eq (name, par) in
            M.ret (Ret arg)
        | _ -> M.fail ""
        end) ;
      (function () ->
